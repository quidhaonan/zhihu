<template>
  ...
  <div class="lazy-more" v-show="newsList.length!==0" ref="loadMore">
    <van-loading size='12px'>正在加载中</van-loading>
  </div>
</template>
...
<script>
    ...
    import { queryNewsBefore, queryNewsLatest } from "../api/index";
        ...
        onMounted(()=>{
            ...
            const ob=new IntersectionObserver(async (changes)=>{
                ...
                if(item.isIntersecting){
                    const result=await queryNewsBefore(state.newsList[state.newsList.length-1]['date'])
